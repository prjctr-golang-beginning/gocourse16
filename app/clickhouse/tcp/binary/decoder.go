...
func NewDecoder(r io.Reader) *Decoder {
	return &Decoder{
		input: r, // TODO: replace with io.TeeReader(r, buf)
		buf:   &bytes.Buffer{},
	}
}
...
func (decoder *Decoder) Uvarint() (uint64, error) {
	uv, err := binary.ReadUvarint(decoder)
	return uv, err
}
...
func (decoder *Decoder) UInt64() (uint64, error) {
	/**/ b := decoder.scratch[:8]
	if _, err := decoder.input.Read(b); err != nil {
		return 0, err
	}
	/**/ decoder.buf.Write(b)
...
func (decoder *Decoder) ReadByte() (byte, error) {
	...
	/**/ decoder.buf.WriteByte(decoder.scratch[0])
	return decoder.scratch[0], nil
}

func (decoder *Decoder) ResetBuf() {
	decoder.buf = &bytes.Buffer{}
}

func (decoder *Decoder) FlushBufBytes() []byte {
	defer decoder.ResetBuf()
	return decoder.buf.Bytes()
}
