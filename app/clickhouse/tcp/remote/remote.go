...
func Open(opt *Options) (*clickhouse, error) {
	opt.setDefaults()
	return &clickhouse{
		opt:  opt,
...
func (ch *clickhouse) BroadcastTo(ctx context.Context, consumer io.Writer) error {
...
func (ch *clickhouse) Send(ctx context.Context, packet []byte) error {
	conn, err := ch.acquire(ctx)
	if err != nil {
		return err
	}
	if err := conn.send(ctx, packet, ch.consumer); err != nil {
		ch.release(conn, err)
		return err
	}
	ch.release(conn, nil)
